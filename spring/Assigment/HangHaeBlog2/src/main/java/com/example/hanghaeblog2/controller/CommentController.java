...
public class CommentController {
    ...
    public ResponseEntity<CommentResponseDto> postComment(@RequestBody CommentRequestDto requestDto, @AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long id) {
        return commentService.postComment(requestDto, userDetails.getUsername(), id);
    }

    // 댓글 수정
    @PutMapping("/comment/{comment}")
    public ResponseEntity<CommentResponseDto> updateComment(@AuthenticationPrincipal UserDetailsImpl userDetails, @RequestBody CommentRequestDto requestDto, @PathVariable Long comment) {
        return commentService.updateComment(userDetails.getUsername(),requestDto,comment);
    }

    // 댓글 삭제
    @DeleteMapping("/comment/{comment}")
    public ResponseEntity<statusResponseDto> deleteComment(@AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long comment) {
        return commentService.deleteService(userDetails.getUsername(), comment);
    }

    // 댓글 좋아요
    @GetMapping("/comment/like/{comment}")
    public ResponseEntity<statusResponseDto> postLike(@AuthenticationPrincipal UserDetailsImpl userDetails, @PathVariable Long comment) {
